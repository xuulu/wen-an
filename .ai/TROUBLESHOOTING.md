...
## 站点设置上传图片超过 2MB 被拒（尺寸限制）

- **症状：** 后台上传较大的 logo / OG 图报"图片需小于 2MB"。
- **根本原因：** 上传路由写死了 2MB 硬限制，未做压缩处理。
- **已验证的解决方案：** 上限放宽到 10MB，并引入 `sharp`（显式加入 package.json）压缩：位图（PNG/JPG/WebP/ICO）最长边缩到 icon 512 / 其他 1200、JPEG/WebP 质量 80、PNG palette 量化；压缩后不小于原图则保留原字节；GIF 动画 / SVG 文本格式不压缩。实测 9.25MB 高熵 JPG → 300KB（1200×880）。
- **验证方式：** 上传 >2MB 图片成功；storage 内文件体积大幅下降且 GET 正常。
- **适用版本 / 环境 / 条件：** 站点设置图片上传；GIF/SVG 不做位图压缩。

## 首页跑马灯左右两端出现白边 / 缺口

- **症状：** 深色跑马灯条左右两端透出页面背景（**浅色主题下尤其明显**；深色/霓虹主题下被深色 body 掩盖，易漏检）。
- **根本原因：** mask-image 直接加在整条容器上，连深色背景一起在两端渐隐。
- **已验证的解决方案：** 背景条（外层）不做 mask、完整铺满；mask 只加在内层「滚动文字」的 overflow 容器上（`absolute inset-0 overflow-hidden [mask-image:...]` 包住 track）。
- **验证方式：** 必须在浅色主题下截图确认背景条左右铺满、仅文字边缘渐隐（深色主题验证会误判）。
- **适用版本 / 环境 / 条件：** 所有带边缘渐隐的通栏组件；改完务必切浅色主题复查。
