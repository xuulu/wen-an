...
### 前台
- **首页文案墙**：分类筛选、关键词搜索（标题/正文/类目 ILIKE 模糊匹配）、一键复制、客户端分页（每页 50 条，跳页）
...
- **批量导入**：粘贴 JSON 或 `标题|正文|日期` 竖线分隔文本，格式自动识别、逐条校验、相似度去重跳过（兼容旧格式，标签字段自动忽略）
...
| `wenan_copy_items` | 文案（`user_id IS NULL` 为公共预置；标签数据见迁移归档表 `wenan_copy_tags_archive`） |
